...
# ==========================================
@tasks.loop(time=datetime_time(hour=14, minute=0))
async def controllo_finanziamenti():
    ...
    for pagato in risultati["pagati"]:
        ...
        embed.add_field(name="📅 Giorni Rimanenti", value=f"{rimanenti} giorni" if rimanenti > 0 else "🏁 ESTINTO!", inline=True)
        embed.set_footer(text="Riscossione Automatica")
        await invia_log_finanziario(guild, embed)

    # 2. LOG DEI PAGAMENTI PARZIALI
    ...
    try:
        with db.cursor() as cursor:
            ...
            for f in finanziamenti:
                ...
                cursor.execute("SELECT bank FROM public.users WHERE id = %s;", (user_id,))
                res = cursor.fetchone()
                saldo_banca = res[0] if res else 0
                
                if saldo_banca >= quota_totale_oggi:
                    cursor.execute("UPDATE public.users SET bank = bank - %s WHERE id = %s;", (quota_totale_oggi, user_id))
                    nuovo_debito = 0
                    ...
                    cursor.execute("UPDATE public.users SET bank = 0 WHERE id = %s;", (user_id,))
...
class BottoneAzioneTelefono(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.azione_id.startswith("social_reg_"):
            ...
            view = discord.ui.View()
            view.add_item(BottoneMettiLike(p_id, user_id, self.azione_id))
            view.add_item(BottoneRitornoHome(user_id))
            await interaction.followup.send(embed=embed, view=view, ephemeral=True)

        elif self.azione_id == "wa_add_contatto":
...
# Corretta ereditarietà posizionandola sotto il suo genitore reale
...
# Alias aggiuntivo richiesto dal sistema
class BottoneRitornoHome(BottoneHome):
    pass
...
async def crea_finanziamento(
    ...
    quota_giornaliera = round(prezzo_totale / duration_giorni if durata_giorni > 0 else prezzo_totale, 2)
...
async def stato_finanziamento(interaction: discord.Interaction):
    ...
    for idx, f in enumerate(finanziamenti, 1):
        ...
        embed.add_field(name=f"📊 #{idx} | {desc}", value=valore_field, inline=False)
...
# ==========================================
# BACKGROUND TASK: CONTROLLO SCADENZE
# ==========================================
@tasks.loop(time=datetime_time(hour=14, minute=0))
    ...
    for user_id, targa, tipo, giorni in lista_avvisi:
        try:
            ...
            if user:
                ...
                embed_dm = discord.Embed(title="⚠️ Avviso Scadenza Veicolo", description=desc_testo, color=colore_embed, timestamp=datetime.now())
...
RUOLO_REVISIONE_1_ID = 1253460183053504582  
RUOLO_REVISIONE_2_ID = 1257782342504812688  
...
@app_commands.describe(targa="La targa del veicolo da modificare", azione="Scegli se aggiungere o rimuovere la modifica", modifica="Il nome della modifica (es. Motore Step 3, Turbo, Estetica)")
...
async def veicolo_mod(interaction: discord.Interaction, targa: str, azione: str, modifica: str):
    user_roles_ids = [role.id for role in interaction.user.roles]
    if RUOLO_REVISIONE_1_ID not in user_roles_ids and RUOLO_REVISIONE_2_ID not in user_roles_ids:
        return await interaction.response.send_message("⛔ Non hai il ruolo lavorativo adatto (Meccanico) per eseguire questa operazione.", ephemeral=True)
    ...
    try:
        ...
        if azione == "aggiungi":
            if modifica_pulita.lower() in [m.lower() for m in lista_modifiche]:
                cur.close()
                conn.close()
                return await interaction.followup.send(f"⚠️ La modifica **{modifica_pulita}** risulta già installata su questo veicolo!", ephemeral=True)
            ...
            if not trovata:
                cur.close()
                conn.close()
                return await interaction.followup.send(f"❌ La modifica **{modifica_pulita}** non è attualmente installata su questo veicolo.", ephemeral=True)
        ...
        embed = discord.Embed(title=titolo_embed, description=descrizione_embed, color=colore_embed, timestamp=datetime.now())
        ...
        print(f"[ERROR] Errore modifiche veicolo: {e}")
...
async def bonifico(interaction: discord.Interaction, utente: discord.Member, ammontare: int):
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT bank FROM public.users WHERE id = %s;", (sender_id,))
        ...
        cur.execute("UPDATE public.users SET bank = bank - %s WHERE id = %s", (ammontare, sender_id))
        cur.execute("UPDATE public.users SET bank = bank + %s WHERE id = %s", (ammontare, receiver_id))
        conn.commit()
        successo = True

        embed = discord.Embed(title="🏦 BONIFICO BANCARIO ESEGUITO", color=discord.Color.blue(), timestamp=datetime.now())
        ...
        if 'conn' in locals() and conn: conn.rollback()
        print(f"[ERROR] Errore bonifico: {e}")
    ...
    if successo:
        try:
            ...
            print(f"[ERROR] Impossibile inviare il log del bonifico: {log_error}")
...
async def on_ready():
    ...
    if not controllo_scadenze_veicoli.is_running():
        ...
        print("🚘 [TASK] Controllo scadenze veicoli avviato!")
    
    try:
        bot.add_view(SchermataHomeGrigliaView(user_id=None, apps=None))
        print("📱 [VIEW] Interfaccia persistente Telefono caricata!")
    except Exception as e:
    ...
    try:
        ...
        print(f"🔄 Sincronizzati {len(synced)} comandi nel server {GUILD_ID}!")
    except Exception as e:
...
async def check_guild(interaction: discord.Interaction):
    if interaction.guild_id not in ALLOWED_GUILDS:
        await interaction.response.send_message("❌ Questo bot non è authorized in questo server.", ephemeral=True)
        return False
    return True


# ================= CONFIGURAZIONE FLASK FOR RENDER =================
app = Flask("")
